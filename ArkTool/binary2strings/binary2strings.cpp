...
vector<std::tuple<string, string, std::pair<size_t, size_t>>> extract_all_strings_(const unsigned char buffer[], size_t buffer_size, size_t min_chars)
{
	// Process the specified binary buffer and extract all strings
	size_t offset = 0;
	vector<std::tuple<string, string, std::pair<size_t, size_t>>> r_vect;
	vector<float> proba_interesting_vect;
	vector<float> proba_interesting_avg_vect;
	extracted_string* s;
	...
	while (offset + min_chars <= buffer_size)
	{
		...
		if (s)
		{
			...
			// Add the new string
			r_vect.push_back(
				tuple<string, string, std::pair<size_t, size_t>>(
					s->get_string(),
					s->get_type_string(),
					std::pair<size_t, size_t>(s->get_offset_start(), s->get_offset_end())
					)
